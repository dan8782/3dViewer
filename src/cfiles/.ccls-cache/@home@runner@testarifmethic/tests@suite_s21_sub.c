...
START_TEST(sub_2) {
  ...
  //Короче почему-то неправильно работает вычитание, разобраться с этим
  int our_result_status = s21_sub(decimal, decimal2, &decimal3);
  printf("%u \n%u \n%u \n%u\n", decimal3.bits[0], decimal3.bits[1],
         decimal3.bits[2], decimal3.bits[3]);
  ck_assert_int_eq(decimal3.bits[0], 1);
...
END_TEST
START_TEST(sub_8) {
  // 3961.4081257132168796771975168 - 0.0000000000000000000000000001 =
...
START_TEST(sub_9) {
  // 3961.4081257132168796771975168 - 0.0000000000000000000000000008 =
  // 3961.4081257132168796771975168
  s21_decimal decimal;
  s21_decimal decimal2;
  s21_decimal decimal3;
...
START_TEST(sub_10) {
  // 3961.4081257132168796771975168 - 0.0000000000000000000000008128 =
  // 3961.4081257132168796771975160
  s21_decimal decimal;
  s21_decimal decimal2;
  s21_decimal decimal3;
  decimal_init(&decimal);
