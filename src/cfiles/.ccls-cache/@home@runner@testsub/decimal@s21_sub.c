...
void s21_my_add_sub(int value_1_degree, int value_2_degree, s21_decimal value_1,
  ...
  for (int i = 0; i < value_1_degree - value_2_degree; i++) {
    ten_struct.bits[3] *= 10;
  }
  binary_multiply(multiply_buffer, ten_struct, &value_2);
  printf("%u\n", value_2.bits[3]);
  if (is_add) {
    binary_sum(value_1, value_2, result);
  } else {
...
int get_degree(s21_decimal decimal) {
  ...
  decimal_init(&buffer);
  for (int i = 16; i < 24; i++) {
    if (is_bit(95 + i, decimal)) {
      set_bit(i - 16, &buffer);
    }
  }
  degree = buffer.bits[3];
  return degree;
}
...
void set_bit(int position, s21_decimal *result) {
  if (0 <= position && position <= 31) {
    result->bits[3] = result->bits[3] | (1 << position);
  }
  if (32 <= position && position <= 63) {
    result->bits[2] = result->bits[2] | (1 << (position - 32));
  }
  if (64 <= position && position <= 95) {
    result->bits[1] = result->bits[1] | (1 << (position - 64));
  }
  if (96 <= position && position <= 130) {
    result->bits[0] = result->bits[0] | (1 << (position - 96));
  }
}
